void resolution_sub34_multipad() {
...
//   std::string sub = "3";
  ...
  std::string filePath = "/home/oleksii/cbmdir/working/qna/aXmass/root/resolutions/sub" + sub + "/";

  MultiPicture* mpic = new MultiPicture(3, 2);
  for(int i=0; i<3; i++) {
    for(int j=0; j<2; j++) {
      TFile* fileIn = TFile::Open((filePath + quantity.at(j) +  "_" + sub + "sub.psd." + setup.at(i) + ".root").c_str());
