...
//   bool average_comp{true}; std::vector<std::string> components{"ave"}; std::string avestatus = ".ave";
  ...
  std::string fileName = "/home/oleksii/cbmdir/working/qna/inv_mass_flow/" + evegen + "/dv1dy/dv1dy.imf." + evegen + "." + particle + ".root";
  ...
  axes.at(kSlice).sim_name_ = "ReconstructedParticles_pT";
  axes.at(kProjection).sim_name_ = "RecEventHeader_centrality_tracks";
  axes.at(kSlice).reco_name_ = "pT";
  axes.at(kProjection).reco_name_ = "centrality";
  ...
  auto* dc = (Qn::DataContainer<Qn::StatDiscriminator,Qn::Axis<double>>*)fileIn->Get<Qn::DataContainer<Qn::StatDiscriminator,Qn::Axis<double>>>("v1mcmatch_intercept.x1x1");
  if(dc==nullptr) {
  ...
  for(auto fc : fitcoeffs) {
    ...
    if(evegen == "dcmqgsm") {
      if(particle == "lambda") {
        if(fc == "slope")     { y_lo = -0.1; y_hi = 0.35; }
    ...
    if(average_comp) {
      v1_est.resize(1);
      v1_est.at(0) = DoubleDifferentialCorrelation( fileName.c_str(), {("v1imf_" + fc + ".ave").c_str()} );
      v1_est.at(0).SetMarker(kFullSquare);
      v1_est.at(0).SlightShiftProjectionAxis(1);
    } else {
      v1_est.resize(2);
      v1_est.at(0) = DoubleDifferentialCorrelation( fileName.c_str(), {("v1imf_" + fc + ".x1x1").c_str()} );
      v1_est.at(1) = DoubleDifferentialCorrelation( fileName.c_str(), {("v1imf_" + fc + ".y1y1").c_str()} );
      v1_est.at(0).SetMarker(kFullSquare);
    ...
    std::vector<DoubleDifferentialCorrelation> v1_ref;
    if(average_comp) {
      v1_ref.resize(1);
      v1_ref.at(0) = DoubleDifferentialCorrelation( fileName.c_str(), {("v1mcmatch_" + fc + ".ave").c_str()} );
      v1_ref.at(0).SetMarker(-1);
    } else {
      v1_ref.resize(2);
      v1_ref.at(0) = DoubleDifferentialCorrelation( fileName.c_str(), {("v1mcmatch_" + fc + ".x1x1").c_str()} );
      v1_ref.at(1) = DoubleDifferentialCorrelation( fileName.c_str(), {("v1mcmatch_" + fc + ".y1y1").c_str()} );
      v1_ref.at(0).SetMarker(-1);
      v1_ref.at(1).SetMarker(-2);
    }

    for(auto& vc : v1_ref) {
      vc.Scale(2.);
      vc.RenameAxis(axes.at(kProjection).sim_name_, axes.at(kProjection).reco_name_);
      vc.RenameAxis(axes.at(kSlice).sim_name_, axes.at(kSlice).reco_name_);
      vc.SetIsFillLine();
      vc.SetErrorType(error_mode);
      vc.SetMeanType(mean_mode);
      vc.SetSliceVariable(axes.at(kSlice).title_.c_str(), axes.at(kSlice).unit_.c_str());
    ...
    TLegendEntry* entry;
    entry = leg1->AddEntry("", "REF: reco-tracks, MC-match", "L");
    entry->SetMarkerSize(2);
    entry = leg1->AddEntry("", "EST: reco-tracks, IMF", "P");
    entry->SetMarkerSize(2);
    ...
    for(int iv=0; iv<v1_est.size(); iv++) { // iv: X, Y, ave
      ...
      if(drawOption == kDifference || drawOption == kChi2) vplot.at(iv) = Minus(v1_est.at(iv), v1_ref.at(iv));
      if(drawOption == kChi2) vplot.at(iv).DivideValueByError();
      if(drawOption == kRatio) vplot.at(iv) = Divide(v1_est.at(iv), v1_ref.at(iv));
      ...
      if(drawOption == kPlain) {
        for( auto obj : v1_ref.at(iv).GetProjections() ){
          pic.AddDrawable( obj );
        }
      }
    } // iv
    ...
    if(drawOption != kPlain) pic.CustomizeYRange();
    else                     pic.SetYRange({y_lo, y_hi});
