...
//   std::string fileNameBase = "/home/oleksii/cbmdir/working/qna/rectrackspsi/" + evegen + "/dv1dy.rtp." + evegen;
  std::string fileNameBase = "/home/oleksii/cbmdir/working/qna/rectrackspsi/" + evegen + "/dv1dy.rebinned.rtp." + evegen;

  std::string fileNameWei = fileNameBase + ".wei.root";
  std::string fileNameNow = fileNameBase + ".now.root";
  ...
  for(auto& particle : particles) {
    ...
    for(auto fc : fitcoeffs) {
      ...
      if(average_comp) {
        v1_rec_wei.resize(1);
        v1_rec_wei.at(0) = DoubleDifferentialCorrelation( fileNameWei.c_str(), {(particle + "/v1rec_" + fc + ".RESCALED.ave").c_str()} );
        v1_rec_wei.at(0).SetMarker(kFullSquare);
      } else {
        v1_rec_wei.resize(2);
        v1_rec_wei.at(0) = DoubleDifferentialCorrelation( fileNameWei.c_str(), {(particle + "/v1rec_" + fc + ".RESCALED.x1x1").c_str()} );
        v1_rec_wei.at(1) = DoubleDifferentialCorrelation( fileNameWei.c_str(), {(particle + "/v1rec_" + fc + ".RESCALED.y1y1").c_str()} );
        v1_rec_wei.at(0).SetMarker(kFullSquare);
      ...
      v1_rec_wei.at(0).SlightShiftProjectionAxis(1);
      if(!average_comp) {
        v1_rec_wei.at(1).SlightShiftProjectionAxis(1, 0.5);
      }

      std::vector<DoubleDifferentialCorrelation> v1_rec_now;
      if(average_comp) {
        v1_rec_now.resize(1);
        v1_rec_now.at(0) = DoubleDifferentialCorrelation( fileNameNow.c_str(), {(particle + "/v1rec_" + fc + ".RESCALED.ave").c_str()} );
        v1_rec_now.at(0).SetMarker(kFullSquare);
      } else {
        v1_rec_now.resize(2);
        v1_rec_now.at(0) = DoubleDifferentialCorrelation( fileNameNow.c_str(), {(particle + "/v1rec_" + fc + ".RESCALED.x1x1").c_str()} );
        v1_rec_now.at(1) = DoubleDifferentialCorrelation( fileNameNow.c_str(), {(particle + "/v1rec_" + fc + ".RESCALED.y1y1").c_str()} );
      ...
      v1_rec_now.at(0).SlightShiftProjectionAxis(-1);
      if(!average_comp) {
        v1_rec_now.at(1).SlightShiftProjectionAxis(-1, 0.5);
      }

      auto v1_mc = DoubleDifferentialCorrelation( fileNameNow.c_str(), {(particle + "/v1sim_" + fc + ".psi.ave").c_str()} );
      v1_mc.Scale(2.);
      ...
      if(drawOption == kPlain) {
        ...
        for(int i=0; i<v1_rec_wei.size(); i++) { // i: X, Y, ave
          int j{0}; // j: slice axis
          for( auto obj_R_MC : v1_rec_wei.at(i).GetProjections() ) {
            std::string comp;
            if(v1_rec_wei.size() == 2) {
              if(i==0) comp = "X";
              if(i==1) comp = "Y";
            } else {
              comp = "AVE";
