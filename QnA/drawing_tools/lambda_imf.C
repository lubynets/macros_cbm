...
//                               0.7-axes.at(kProjection).shift_});
  ...
  for(auto& ip : inputs) {
    for(auto& se : ip.subevents_) {
      ...
      std::string fileOutName = "v1.imf." + evegen + "." + pbeam + "agev." + pdg + "." + se + ip.resname_;
      ...
      for(int iEdge=0; iEdge<axes.at(kSelect).bin_edges_.size()-1; iEdge++) {
        ...
        TLegendEntry* entry;
        entry = leg1->AddEntry("", "REC, MC-match", "F");
        entry->SetFillColorAlpha(kBlack, 0.2);
        entry->SetLineColor(kWhite);
        entry->SetFillStyle(1000);

        if(is_imf) entry = leg1->AddEntry("", "REC, inv. mass fit", "P");
        else       entry = leg1->AddEntry("", "REC, all candidates", "P");
