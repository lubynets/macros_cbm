...
//   bool average_comp{false}; std::vector<std::string> components{"x1x1", "y1y1"};
  ...
  std::string fileName = "/home/oleksii/cbmdir/working/qna/rectrackspsi/" + evegen + "/cl.rtp." + evegen + ".root";
  ...
  if(!is_rebin_centrality) {
    fileOutName = "dv1dy.rtp." + evegen + ".root";
  } else {
    fileOutName = "dv1dy.rebinned.rtp." + evegen + ".root";
  }
  if(average_comp) {
    ...
    for (auto& step : steps) {
      for(auto& ws : weightstatus) {
        for(auto& co : components) {
          ...
          if(average_comp) {
            v1_rec_in = Qn::DataContainerStatCalculate(*((Qn::DataContainerStatCollect*) fileIn->Get(("rec/u_" + pa + "_rec_" + ws + "_" + step + ".Q_psi_PLAIN.x1x1").c_str()))) +
                        Qn::DataContainerStatCalculate(*((Qn::DataContainerStatCollect*) fileIn->Get(("rec/u_" + pa + "_rec_" + ws + "_" + step + ".Q_psi_PLAIN.y1y1").c_str())));
            v1_rec_in = v1_rec_in/2.;
          } else {
            v1_rec_in = Qn::DataContainerStatCalculate(*((Qn::DataContainerStatCollect*) fileIn->Get(("rec/u_" + pa + "_rec_" + ws + "_" + step + ".Q_psi_PLAIN." + co).c_str())));
          }
  ...
  //           gr_rec->Fit(frec, "0", "", midrapidity-0.01, fitaxis_hi);
          ...
          v1rec_intercept.Write(("v1rec_intercept." + ws + "." + step + "." + co).c_str());
          v1rec_slope.Write(("v1rec_slope." + ws + "." + step + "." + co).c_str());
        }
      }
    }
  }
